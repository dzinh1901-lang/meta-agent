...
class InMemoryStateStore {
  ...
  async put(collection, id, value) {
    ...
    const record = { id, value, created_at: now, updated_at: now, createdAt: now, updatedAt: now };
    this.seed[collection].push(record);
    return record;
  }
}
...
class PostgresStateStore {
  ...
  async put(collection, id, value) {
    ...
    await this.pool.query(
      `insert into ${this.tableName(collection)} (id, value, created_at, updated_at) values ($1, $2::jsonb, $3, $4)
...
function repositoryList(inputList) {
  ...
  return list.length ? list : loadRegistry().repositories.map((repo) => repo.repository_full_name);
}

function highRiskBlocked(records) {
  return getCollectionRows(records)
    .map((row) => ({ id: row.id, value: row.value || {} }))
    .filter((entry) => entry.value.blocked === true || entry.value.execution_disposition === 'blocked' || entry.value.routing_status === 'blocked_by_policy')
    .map((entry) => entry.id);
}
...
async function runScheduledScan(stateStore, options) {
  ...
  for (const repository of targetRepos) {
    ...
    if (workflow.task_packet?.task_id) await stateStore.put('taskPackets', workflow.task_packet.task_id, workflow.task_packet);
    if (workflow.approval_packet?.approval_id) await stateStore.put('approvalPackets', workflow.approval_packet.approval_id, workflow.approval_packet);
    if (workflow.pending_approval?.queue_id) await stateStore.put('approvalQueues', workflow.pending_approval.queue_id, workflow.pending_approval);
    if (workflow.agent_run?.run_id) await stateStore.put('agentRuns', workflow.agent_run.run_id, workflow.agent_run);
    workflows.push({ repository, run_id: workflow.agent_run?.run_id || null, task_id: workflow.task_packet?.task_id || null, queue_id: workflow.pending_approval?.queue_id || null });
  }

  const scanEventId = stableId('scan', { label: options.scanLabel, at: new Date().toISOString() });
  const audit = { event_type: 'scheduled_scan_executed', scan_label: options.scanLabel, repository_count: targetRepos.length, workflow_count: workflows.length, operator_id: options.operatorId, generated_at: new Date().toISOString(), details: workflows };
  await stateStore.put('auditEvents', scanEventId, audit);
  return { mode: 'scan', generated_at: new Date().toISOString(), summary: `Scheduled scan complete. repositories=${targetRepos.length}, workflows=${workflows.length}`, scan_label: options.scanLabel, scan_audit_event_id: scanEventId, workflows };
}

async function readAllCollections(stateStore) {
  const collections = {};
  for (const collection of COLLECTIONS) collections[collection] = await stateStore.list(collection);
  return collections;
}

async function runAlertCheck(stateStore, options) {
  const collections = await readAllCollections(stateStore);
  const blockedTasks = highRiskBlocked(collections.taskPackets);
  const rejectedDecisions = getCollectionRows(collections.approvalDecisions).filter((row) => ['reject', 'always_reject'].includes((row.value || {}).decision_type)).length;
  const alertPayload = { event_type: 'external_alert', alert_type: 'operations_health', severity: blockedTasks.length > 0 || rejectedDecisions > 0 ? 'critical' : 'warning', repository_count: collections.repositories.length, blocked_scan_count: blockedTasks.length, blocked_approvals: rejectedDecisions, generated_at: new Date().toISOString() };
  ...
  return { mode: 'alert', generated_at: alertPayload.generated_at, summary: `Alert emitted: severity=${alertPayload.severity}, blocked_scans=${blockedTasks.length}, blocked_approvals=${rejectedDecisions}`, alert_id: alertId, payload: alertPayload };
}
...
async function runBackupOrRollback(mode, stateStore, options) {
  ...
  if (mode === 'backup') {
    const backup = buildBackupRun({ repository: options.repository, scope: options.scope || 'full', retentionDays: options.retentionDays || 30, operatorId: options.operatorId, runCorrelationId: options.runCorrelationId });
    await stateStore.put('backupPlans', backup.backup_plan_id, backup);
    await stateStore.put('auditEvents', eventId, { event_type: 'backup_plan_prepared', repository: options.repository, backup_plan_id: backup.backup_plan_id, operator_id: options.operatorId, recorded_at: new Date().toISOString() });
  ...
  const rollback = buildRollbackRun({ repository: options.repository, targetRunId: options.targetRunId, triggerEventId: options.triggerEventId, operatorId: options.operatorId });
  await stateStore.put('agentRuns', rollback.rollback_plan_id, rollback);
  await stateStore.put('auditEvents', eventId, { event_type: 'rollback_plan_prepared', repository: options.repository, rollback_plan_id: rollback.rollback_plan_id, operator_id: options.operatorId, recorded_at: new Date().toISOString() });
...
async function runReport(stateStore) {
  const rows = await readAllCollections(stateStore);
  const alerts = getCollectionRows(rows.auditEvents).filter((row) => ['external_alert', 'scheduled_scan_executed'].includes((row.value || {}).event_type));
  return { mode: 'report', generated_at: new Date().toISOString(), summary: `Collections: repos=${rows.repositories.length}, tasks=${rows.taskPackets.length}, approvals=${rows.approvalPackets.length}, backups=${rows.backupPlans.length}, alerts=${alerts.length}`, latest_alerts: alerts.slice(-10), scan_collection_size: rows.approvalQueues.length };
}
...
async function loadSeedFromFile(seedPath) {
  ...
  const result = {};
  for (const [key, value] of Object.entries(parsed)) result[key] = Array.isArray(value) ? value.map((record) => toRecord(record)).filter(Boolean) : [];
  return result;
}
...
async function run() {
  ...
  const common = { scanLabel: args.scanLabel, repository, targetRunId: args.targetRunId, triggerEventId: args.triggerEventId, scope: args.scope || 'full', retentionDays: Number.isFinite(retentionDays) ? retentionDays : 30, operatorId, actionType: args.actionType, source: usePostgres ? 'postgres' : 'memory' };
  const result = mode === 'scan' ? await runScheduledScan(stateStore, common) : mode === 'alert' ? await runAlertCheck(stateStore, common) : mode === 'backup' ? await runBackupOrRollback('backup', stateStore, common) : mode === 'rollback' ? await runBackupOrRollback('rollback', stateStore, common) : await runReport(stateStore, common);
  if (args.json) return console.log(JSON.stringify(result, null, 2));
  printSummary(result);
  if (isSensitiveIntegrationAction(common.actionType) && mode === 'scan') console.log(`Note: mode uses integration action ${common.actionType} only in approval-gated planning path.`);
}
